            ...
            double sumPltForceZ = 0.0;
		        ...
		        for(unsigned interactionCounter = 0; interactionCounter < pltmaxConn; interactionCounter++) {
		          ...
		          //check if tendril pulls a plt
		           if (tndrlNodeId[storageLocation + interactionCounter]!=maxIdCount && tndrlTypeId[storageLocation + interactionCounter]==1){//note this happens only if plt-plt interaction is on
